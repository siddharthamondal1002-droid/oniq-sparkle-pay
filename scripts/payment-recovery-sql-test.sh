...
# managed PG* variables and connects over a unix socket under /tmp, so a missing
# flag cannot silently reach production.
#
...
DIR=/tmp/pgtest-payrec
PORT=55432
PSQL=(env -u PGHOST -u PGPORT -u PGUSER -u PGPASSWORD -u PGDATABASE
      psql -X -v ON_ERROR_STOP=1 -h "$DIR" -p "$PORT" -U test -d payrec)
...
wait
OPENED=$(cat "$DIR"/case.* | grep -c '^opened$' || true)
