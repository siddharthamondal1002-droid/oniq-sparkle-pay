...
//
// NOTHING HERE CALLS A MODEL OR GOOGLE. Phase 1 has no AI and no Healthcare
// API; `adapterFor` returns the null adapter and the isolation test fails if a
// provider host or a model helper appears in this file.
//
...
import { readHealthFlags, type HealthFlags } from "../_shared/health/flags.ts";
import {
  CATEGORY_FOR_KIND,
...
async function loadConsents(ctx: Ctx): Promise<ConsentRow[]> {
  const { data } = await ctx.admin
    ...
    .select(
      "id, purpose, data_categories, recipient, status, start_time, expiry_time, version, source, revoked_at, created_at, terms_version, notice_locale",
    )
...
/** The covering consent for a purpose+category, or a refusal naming both. */
async function requireConsent(
  ...
  const recipient = RECIPIENT_FOR_PURPOSE[purpose];
...
async function ledgerConsent(
  ...
  await ctx.admin.from("consent_records").insert({
    ...
    notice_version: CONSENT_TERMS_VERSION,
...
async function actConsentsGrant(ctx: Ctx, body: Record<string, unknown>): Promise<Response> {
  ...
  if (!PHASE1_GRANTABLE_PURPOSES.includes(purpose)) {
    return refuse(ctx, {
      status: 400,
      reason: "bad_input",
  ...
  await ledgerConsent(ctx, purpose, categories, "granted", locale);
...
async function actConsentsRevoke(ctx: Ctx, body: Record<string, unknown>): Promise<Response> {
  ...
  const { data } = await ctx.admin
    ...
    .eq("user_id", ctx.userId)
    .eq("status", "active")
    .select("id, purpose, data_categories, recipient, version, notice_locale")
    .maybeSingle();
  if (!data) return refuse(ctx, { status: 404, reason: "not_found" });
  await ledgerConsent(
    ctx,
...
async function actExport(ctx: Ctx): Promise<Response> {
  const [records, documents, consents, auditRows] = await Promise.all([
    ctx.admin
      .from("health_records")
      .select(
...
Deno.serve(async (req: Request) => {
  ...
  const { flags, environment } = await readHealthFlags(admin);
