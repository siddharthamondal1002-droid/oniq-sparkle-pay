...
Deno.serve(async (req) => {
  ...
  // spend guard: music_jobs IS the daily-cap ledger. Both counts further down
  ...
  // by deleting their songs in a loop: unlimited generation on the
  ...
  if (body.action === "delete") {
    ...
    if (!id) return json(400, { error: "Which song?" });
    ...
    // it and any signed-in person could delete anyone's song by id.
    const { data: mine, error: readErr } = await admin
      ...
      .eq("user_id", user.id)
      .maybeSingle();
    if (readErr) return json(500, { error: "Couldn't delete that song" });
    // The same answer for "not yours" and "not there", so this cannot be used
    // to ask whether an id exists.
    if (!mine) return json(404, { error: "That song is not there" });
    ...
    const { error: markErr } = await admin
      ...
      .eq("id", id)
      .eq("user_id", user.id);
    if (markErr) return json(500, { error: "Couldn't delete that song" });
    ...
    if (path) {
      ...
      if (rmErr) console.warn("[music-generate] delete left an orphan", path, rmErr.message);
    }
