...
export type BenchmarkTask = {
  ...
  readonly toolCatalog: readonly BenchmarkToolSpec[];
};
...
function systemPrompt(task: BenchmarkTask, arm: BenchmarkArm): string {
...
export async function runE003BArm(
  ...
  const transcript: { role: "user" | "assistant" | "tool"; content: string }[] = [
    { role: "user", content: JSON.stringify({ task: task.prompt, evidence: task.initialEvidence }) },
  ];
  let modelCalls = 0;
  let toolCalls = 0;
  let inputTokens = 0;
