...
export type Directness =
  | "experimentally_verified" // ONIQ ran a discriminating experiment and read the outcome
...
/**
 * `experimentally_verified` IS THE SECOND ROUTE TO KNOWLEDGE, and adding it is
 * the whole reason ONIQ is not confined to what it can fetch.
 *
 * Every other rung describes a document someone else wrote. This one describes
 * a DISCRIMINATING EXPERIMENT ONIQ ran itself: a procedure whose competing
 * hypotheses predict DIFFERENT observable outcomes, executed, with a control,
 * and the outcome read. `conventions.ts` is the first one. That is why the
 * network being unreachable from this container is a limit on ONIQ's reading
 * and not a limit on ONIQ's knowing.
 *
 * WHY THE WEIGHT IS 1 AND NOT MORE, since it is plainly more direct than
 * reading a page. The scale is capped at 1 by construction — every factor in
 * `evidenceWeight` is in [0,1] and `promotion.ts` sets its threshold from that
 * arithmetic — so a rung above 1 would silently rewrite the promotion table.
 * What distinguishes an experiment is therefore NOT more weight; it is
 * `conflict.ts`'s `experimental_precedence`, which lets a measurement of a
 * system ONIQ controls outrank a document ABOUT that system. A number would
 * have made it merely heavier; a strategy makes it decisive, and only where it
 * should be.
 *
 * TWO RUNGS THAT WERE PROPOSED AND ARE DELIBERATELY ABSENT, because both
 * already exist on their proper axis and folding them in here would weaken
 * them:
 *
 *   EXTRACTED       is `ExtractionMethod`, above. It answers "how did the claim
 *                   come out of the artefact", which is orthogonal to "how far
 *                   is the artefact from an observation" — a model extraction
 *                   from a fetched page and a direct quotation from a fetched
 *                   page are the same directness and very different evidence.
 *                   Collapsing the axes would lose exactly that distinction.
 *   CROSS_VERIFIED  is a property of the RECORD, not of one evidence item:
 *                   independent sources agreeing. `minIndependentSources` and
 *                   the additive `scoreConfidence` already carry it. As a
 *                   directness it would let a SINGLE item claim corroboration
 *                   it cannot have — pseudo-provenance, which is the failure
 *                   this whole type exists to prevent.
 */
export const EXPERIMENTAL_DIRECTNESS: Directness = "experimentally_verified";
...
export const DIRECTNESS_WEIGHT: Readonly<Record<Directness, number>> = {
  // Capped at 1 with `fetched`, on purpose — see EXPERIMENTAL_DIRECTNESS above.
  experimentally_verified: 1,
  fetched: 1,
  search_snippet: 0.6,
  spec_cited: 0.5,
  derived: 0.9,
