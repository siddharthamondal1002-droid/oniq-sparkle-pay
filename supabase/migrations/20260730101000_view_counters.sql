-- P3: write-time counters (no count(*) per grid render) + backfill.
...
-- clips.view_count already exists.

-- Increment ONLY on first insert (a repeat view is an ON CONFLICT UPDATE,
-- which never fires this AFTER INSERT trigger).
...
-- Backfill history from the legacy per-surface tables, excluding rows the
-- new rules would never have recorded (owner self-views, blocked pairs).
...
-- Recompute counters from truth (the backfill INSERTs above also fired the
-- bump trigger, so set counters absolutely rather than additively).
...
-- Live counts for an open reel player: let clients subscribe to clip rows.
DO $$ BEGIN
