-- Append-only moderation audit log. Separate schema, admin-only SELECT,
-- INSERT only via SECURITY DEFINER writer, UPDATE/DELETE blocked by
-- triggers. Stores ids/references and reasons only — never content.
...
-- No INSERT/UPDATE/DELETE policies: writes only via the definer function.
