-- ============================================================================
-- ONIQ HEALTH — PHASE 2 (dark). Owner brief 2026-09-08 §83: the Health AI
-- safety gateway and document intelligence, SYNTHETIC PROVIDER ONLY.
-- NOT APPLIED at the time of writing. Applies AFTER
-- 20260908120000_oniq_health_phase1.sql; the go sequence is
-- docs/health/04-decisions-for-owner.md §A-2.
--
-- WHAT CHANGES, stated once:
--   * health_config gains the AI columns: which provider (CHECK: synthetic
--     only), which model, the two daily caps (DEFAULT 0 = refuse until the
--     owner sets them), the admin-verification switch, and the twelfth flag
--     from §83C, provider_sharing_enabled. All off.
--   * health_records gains the candidate/rejected statuses, a confidence, a
--     rule that an extracted row is numeric (a free-text candidate would be
--     the document talking), and a candidate index.
--   * health_documents gains the classification hint and extraction state.
--   * health_consents gains jurisdiction and the rule that a row's terms
--     version DISCLOSED its recipient — mirrors DISCLOSED_RECIPIENTS_BY_TERMS
--     in consent.ts, so a consent given for ONIQ can never be re-read as one
--     for Google.
--   * health_audit accepts the six new actions.
--   * health_ai_requests is the RECEIPT: one row per request, written before
--     the provider runs, ids and counts only. It is the cap ledger, so it
--     survives account deletion with user_id set null (the house count must
--     not reset when an account is deleted and recreated), is never hard
--     deleted by purge (purge marks it and blanks the manifest), and every
--     refusal it stores is a CLOSED code from types.ts.
-- Every constraint added here is NAMED, so the next phase can drop it by
-- name instead of guessing Postgres's auto-name.
-- ============================================================================
...
create table if not exists public.health_ai_requests (
  ...
  contract_code text check (contract_code is null or contract_code in (
    ...
    'interpretation_without_source','confidence_range')),
...
-- No write policy for anyone but the service role, and no UAE guard: the row
-- holds ids, counts and codes, never a reading, so it is a ledger rather than
-- health data — which is also why it survives deletion and purge.
