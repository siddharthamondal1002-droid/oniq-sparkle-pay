-- ============================================================================
-- ONIQ HEALTH — PHASE 1 (dark). Owner brief, 2026-09-08:
-- "ONIQ HEALTH — FULL GOOGLE HEALTHCARE INTEGRATION", built as an isolated,
-- feature-flagged health-data domain. NOT APPLIED at the time of writing;
-- the go sequence is docs/health/04-decisions-for-owner.md §A.
--
-- SHAPE, stated once:
--   * Every user table: RLS on; authenticated may SELECT its own rows and
--     nothing else; every write is the service role through `health-api`,
--     which filters on user_id and audits.
--   * Every user table: FK to profiles ON DELETE CASCADE (account deletion
--     removes the rows), the UAE two-axis write guard already used by the
--     Vitals tables (health_write_guard + a RESTRICTIVE policy), and a
--     provenance column that cannot be empty.
--   * health_audit is a sha256 hash chain, appended only by a SECURITY DEFINER
--     function that only service_role may execute, and it never carries
--     content — kinds, counts and ids only.
--   * health_config is the server half of the eleven flags, all false. A
--     missing row is "off" (the function fails closed).
--   * The bucket is private with NO policy for authenticated: bytes move only
--     through signed URLs the function mints.
-- ============================================================================
...
-- 3. DOCUMENTS (before records, which reference them) -------------------------
...
-- No INSERT/UPDATE/DELETE policy for anyone: rows arrive only through
-- health_append_audit(), and nothing edits them afterwards.
...
-- 7. RETENTION SWEEP (marks; bytes are the Phase 2 worker's) -------------------
...
-- 8. THE BUCKET: private, server-only --------------------------------------------
insert into storage.buckets (id, name, public)
values ('health-documents', 'health-documents', false)
on conflict (id) do nothing;
-- Deliberately NO storage.objects policy for authenticated. Uploads use a
-- signed upload URL and reads a 60-second signed URL, both minted by
-- health-api after the ownership check, and both audited.
