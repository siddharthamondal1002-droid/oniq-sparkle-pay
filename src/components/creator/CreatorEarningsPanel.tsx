 ...
 *
 * A FAILED balance read is NOT a zero balance. Before 2026-08-19 a network or
 * RLS hiccup on creator_balance rendered ₹0 accrued / ₹0 available — telling a
 * creator they had earned nothing. The balance query now throws on error and
 * this panel shows an explicit retry in place of the money numbers.
 */
...
import { viewState } from "@/lib/queryView";
...
export function CreatorEarningsPanel({ userId }: { userId: string }) {
  ...
  const {
    ...
    queryFn: async () => {
      const { data, error } = await sb.rpc("creator_balance", { _uid: userId });
  ...
  // A balance is always a record (zeros included), so it is never "empty" —
  // only loading, failed, or present.
  const balView = viewState({ isLoading: balLoading, isError: balError }, false);
  ...
  return (
    <section className="rounded-2xl bg-surface p-4">
      ...
      {balView === "error" ? (
        <QueryError
          onRetry={() => refetchBal()}
          label="Couldn't load your earnings right now — this is a connection problem, not a zero balance."
          busy={balLoading}
        />
      ) : balView === "loading" ? (
        <div className="grid grid-cols-2 gap-3">
          <div className="h-[86px] animate-pulse rounded-xl bg-background/60" />
          <div className="h-[86px] animate-pulse rounded-xl bg-background/60" />
        </div>
      ) : (
        <div className="grid grid-cols-2 gap-3">
        ...
        <p className="mt-3 text-xs text-muted-foreground">
          ...
          {rate.refundBp >= BREAKEVEN_REFUND_BP && (
            <span className="ms-1 inline-flex items-center gap-1 text-amber-500">
              <AlertTriangle className="h-3 w-3" /> past the{" "}
              {(BREAKEVEN_REFUND_BP / 100).toFixed(1)}% break-even
            </span>
          )}
        </p>
      )}
      ...
      {notices.length > 0 && (
        <ul className="mt-3 space-y-1 text-xs text-muted-foreground">
          {notices.map((n) => (
            <li key={n.id}>
              −{formatPaise(n.amount_paise)} · {n.reason} · {n.period ?? ""} — refunded
              subscriptions are money that was never earned, not a penalty. Reply to dispute; a
              human answers within 7 days.
            </li>
          ))}
        </ul>
      )}
