...
function parseGenie(raw: string): { url: string } | { error: string } {
  const q = raw.trim();
  ...
  if (mode.startsWith("flight") || mode === "fly") {
    ...
    const from = encodeURIComponent(toMatch[1].trim());
    const to = encodeURIComponent(toMatch[2].trim());
    return { url: `https://www.google.com/travel/flights?q=Flights%20from%20${from}%20to%20${to}` };
  }
  ...
  if (mode.startsWith("train")) {
    return { url: "https://www.ixigo.com/trains" };
  }
  if (mode.startsWith("bus")) {
    return { url: "https://www.redbus.in" };
  }
  if (mode === "ferry") {
    return { url: "https://www.makruzz.com" };
  }
