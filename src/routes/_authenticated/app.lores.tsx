...
import { OniqCanvas, OniqHeader, OniqMediaCard, OniqSectionHeader } from "@/components/oniq";

type LoresSearch = { tab?: "originals" | "stories" | "library" };

export const Route = createFileRoute("/_authenticated/app/lores")({
  // ?tab=stories from the Create launcher (owner mission, 2026-09-03). Anything
  // else is dropped, never thrown on.
  validateSearch: (s: Record<string, unknown>): LoresSearch => ({
    tab:
      s.tab === "originals" || s.tab === "stories" || s.tab === "library"
        ? (s.tab as LoresSearch["tab"])
        : undefined,
  }),
  head: () => ({
    meta: [
...
/** Entrance delays for the collections — literals, so the CSS scan finds them. */
const RISE = ["rise-1", "rise-2", "rise-3", "rise-4", "rise-5"];
...
function LoreCard({ v }: { v: LoreVideo }) {
  ...
  // ONIQ's own file, so its controls may sit over the frame — the rule against
  // layering anything on a player is for third-party embeds. The frame stays
  // black (OniqMediaCard) in every mode.
  const media = v.url ? (
    <>
      <video
        ref={videoRef}
        ...
        onTimeUpdate={(e) => {
          ...
          if (modeRef.current === "preview" && e.currentTarget.currentTime >= PREVIEW_SECONDS) {
      ...
      {mode === "idle" && (
        <div className="absolute inset-0 flex items-center justify-center gap-3 bg-black/30">
          <button
            ...
            className="press rounded-full border border-white/30 bg-black/50 px-4 py-2 text-xs font-semibold text-white backdrop-blur-sm"
    ...
    <div className="grid h-full w-full place-items-center text-[11px] font-semibold uppercase tracking-wider text-white/60">
      in production
    </div>
  );
  ...
  return (
    <OniqMediaCard
      ...
      footer={
        <>
          <div className="font-display text-[15px] leading-tight text-foreground">{v.title}</div>
          ...
          {prov && (
            <details className="mt-2 rounded-xl bg-surface-2 px-2.5 py-1.5">
  ...
  return (
    <OniqCanvas world="lores" className="pb-28">
      <OniqHeader
        eyebrow={
          <span className="inline-flex items-center gap-1.5">
            <Clapperboard className="h-3.5 w-3.5" aria-hidden="true" /> ONIQ Originals
          </span>
        }
        title="Lores 🎬"
        subtitle="stories made in-house — no cap, all ours"
        back="/app"
      />
      ...
      <div
        ...
        // they were not. It sticks just under the status-bar scrim the shell
        // paints, which is why the offset is the inset and not 0.
        className="sticky top-[env(safe-area-inset-top)] z-20 mx-5 mt-4 flex gap-1 rounded-full oniq-glass p-1 shadow-card"
      >
        {TABS.map((t) => (
          <button
            key={t.id}
      ...
      {tab === "originals" ? (
        <div role="tabpanel" id="lores-panel-originals" aria-labelledby="lores-tab-originals">
          ...
          <div className="mx-5 mt-4 flex flex-wrap items-center gap-2 rounded-2xl border border-amber-400/40 bg-amber-400/10 px-3 py-2">
          ...
          {LORE_COLLECTIONS.map((c, ci) => (
            <section key={c.name} className={`mt-7 rise ${RISE[Math.min(ci, RISE.length - 1)]}`}>
              <OniqSectionHeader eyebrow="Collection" title={c.name} />
              <div className="mt-3 grid gap-4 px-5">
                {c.videos.map((v) => (
        ...
        <div
          role="tabpanel"
          id="lores-panel-stories"
          aria-labelledby="lores-tab-stories"
          className="px-5 pt-4"
        >
          <StoryStudio />
        </div>
        ...
        <div
          role="tabpanel"
          id="lores-panel-library"
          aria-labelledby="lores-tab-library"
          className="px-5 pt-4"
        >
          <YourVideos />
        </div>
