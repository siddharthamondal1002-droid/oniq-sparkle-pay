...
**Assessment time:** 2026-09-15 23:07 IST (17:37 UTC)  
...
admin/push access. The default branch is `main`. The pinned merge commit was
authored and committed at `2026-09-15T14:35:52Z`, before the user's stated
`2026-09-15 23:07 IST`; no timestamp anomaly was found. [E01]
...
`78ceab7a170703c903262bae27be0775e592d0d3`;
...
|---|---:|
| Recursive tree entries | 2,896 |
| Blobs | 2,583 |
| TypeScript test files | 418 |
...
**Observed:** The pinned merge commit has no PR-triggered workflow run attached.
Its merged PR #172 head `03b288155d8b94d2fab1b1925009b3ef7a862d4a`
has successful `lint` workflow run 1966. PR evidence reports `npm ci`, blocking
lint, dependency validation, TypeScript, media prerequisites, and 418/418 test
files with 7,334/7,334 tests passing. Formatting said no merge base, so
formatting is not independently claimed. [E11]
...
`selectByCapability()` filters shutdown models. [E03]
...
**Observed:** Ting accepts text plus image/PDF attachments, uses a search-capable
Anthropic route, can fall back to direct Gemini, and returns retrieved sources.
Smart Scout accepts product/location text plus an image, bounds searches and
tokens, requires search on its Gemini fallback, and returns source records.
[E04]
...
|---|---|---|
| E01 | `BASELINE_EVIDENCE_MANIFEST.json`; GitHub repository metadata; commit `b97f6b7`; pinned tree `78ceab7`; root/branch/workflow listings | Identity, access, date, checksum-addressed counts, required docs absent on `main`, acquisition limits |
| E02 | `package.json`; `.github/workflows/lint.yml`; root `AGENTS.md` | Runtime/toolchain and blocking CI policy |
| E03 | `supabase/functions/_shared/modelRegistry.ts::MODEL_REGISTRY`, `capabilityMatch`, `selectByCapability`; indexed call-site search | Model inventory, capabilities, shutdown filtering, selector reachability |
| E04 | `supabase/functions/ting/index.ts`; `smart-scout/index.ts`; `_shared/webRetrieval.ts`; `_shared/searchGuard.ts` | Text/search/multimodal routes, retrieval validation, sources, bounds, historical Scout cost |
...
| E11 | PR #172 metadata; workflow run 1966 on head `03b2881` | CI result and merge relationship |
...
- GitHub merge-commit CI is absent; evidence is the merged PR head run.
