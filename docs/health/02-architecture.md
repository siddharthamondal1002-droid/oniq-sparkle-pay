...
**Changed 2026-09-09, owner directive.** `src/routes/privacy.tsx` now says, verbatim from `src/config/privacy.ts` (`HEALTH_AI_PRIVACY_STATEMENT`): "Health data may be processed by ONIQ's AI-assisted health features when you choose to use them and provide the required consent. AI-assisted features are subject to ONIQ's privacy, security, consent, audit, and safety controls." The same statement sits beside the AI consent on `app.health.consent.tsx` as `health.privacy.ai_processing` in English, Hindi and Bengali — the two translations are counsel-review placeholders, meaning-preserving and unapproved — and the Play declaration's "AI processing" entry mirrors it. `src/health/__tests__/privacyDisclosure.test.ts` pins every copy to the constant, keeps "never sent to any AI" out of every user-facing source in any language, and ties the statement's claims to the code: the AI purpose is grantable for ONIQ only under a terms version that discloses ONIQ, a revoked grant covers nothing, and `RECIPIENT_FOR_PROVIDER` holds no recipient but ONIQ. The consent SENTENCE (`health.consent.ai`, D3) is untouched.

**Phase 3 (owner directive 2026-09-09, later the same day) did what the paragraph below said it would need.** The notice now NAMES the recipient — a sentence directly after the approved statement, `HEALTH_AI_RECIPIENT_SENTENCE` in `src/config/privacy.ts`: "When you use them, the records you ask about are sent to Google Cloud Vertex AI (Gemini), operated by Google, to produce the answer, and are not used to train Google's models." — says what is sent (the records asked about; the Play entry itemises kind, name, value, unit, date) and the not-used-to-train position (a SNIPPET-labelled Vertex data-governance claim in `01`, for counsel to confirm). A new terms version, `health-ai-terms-v2`, discloses `google_vertex`; the migration's CHECK and `consentCovers` both refuse a v1 row for that recipient. Play Data safety declares the AI-processing recipient and the server-side host `aiplatform.googleapis.com`. The same sentence sits beside the AI consent in English, Hindi and Bengali (placeholder translations, labelled). `privacyDisclosure.test.ts` ties the sentence to `RECIPIENT_FOR_PROVIDER`, so the day the registered provider changes, the notice fails the build until it changes with it. Design as built: `05 §17`; measurements: `07`.

**And the thing the old sentence hid is closed, not disclosed.** The Vitals report scan (`supabase/functions/health-scan`) had `ANTHROPIC_API_KEY` configured in production (measured 2026-09-09: an authenticated call with an empty attachment answered `400 attach a report photo or PDF`, a line reached only after the key check), so an uploaded lab report reached Anthropic with no consent step and outside the health audit. Under the Phase 3 directive ("Remove Anthropic from the active Health AI path") the function is a 410 stub — no key, no body, no fetch — the Vitals section that called it is gone, and `anthropicRetired.test.ts` keeps both that way. The consented, audited path on the Health tab is the only AI a health byte reaches. `04 D4`'s "disclose or gate" question is therefore moot for that path; the notice's section 4 still lists the three Anthropic surfaces that remain (Ting, Study, CV), none of which takes health data.
