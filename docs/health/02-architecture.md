...
What Phase 3 still needs before a real provider goes live is unchanged in kind: the notice must NAME the recipient (Google, on Vertex or the paid Gemini API), say which health data is sent and that it is not used to train, a new terms version must disclose that recipient, Play Data safety must declare it, and the tests above change with it. Owner and counsel decision.

**And one thing the old sentence hid, measured 2026-09-09.** The Vitals report scan (`supabase/functions/health-scan`) has `ANTHROPIC_API_KEY` configured in production: an authenticated call with an empty attachment answers `400 attach a report photo or PDF`, which the function only reaches once the key check has passed (no key answers `{configured: false}`). So an uploaded lab report already reaches Anthropic today — ephemeral, rate-limited, forbidden to diagnose, but with no consent step and outside the health audit. The approved statement does not name it, its "required consent" is not asked there, and section 4 of the notice lists three Anthropic surfaces and not this one. Whether to disclose it there or put it behind consent first is counsel's (`04 D4`); this change did not touch the feature.
